...
int CAddrInfo::GetTriedBucket(const std::vector<unsigned char> &nKey) const
{
    ...
    ss1 << nKey << vchKey;
    uint64 hash1 = Hash2(ss1.begin(), ss1.end()).Get64();

    CDataStream ss2(SER_GETHASH, 0);
    std::vector<unsigned char> vchGroupKey = GetGroup();
    ss2 << nKey << vchGroupKey << (hash1 % ADDRMAN_TRIED_BUCKETS_PER_GROUP);
    uint64 hash2 = Hash2(ss2.begin(), ss2.end()).Get64();
...
bool CAddrInfo::IsTerrible(int64 nNow) const
{
    ...
    if (nTime==0 || nNow-nTime > ADDRMAN_HORIZON_DAYS*129600) // not seen in over a month
        return true;

    if (nLastSuccess==0 && nAttempts>=ADDRMAN_RETRIES) // tried three times and never a success
        return true;

    if (nNow-nLastSuccess > ADDRMAN_MIN_FAIL_DAYS*129600 && nAttempts>=ADDRMAN_MAX_FAILURES) // 10 successive failures in the last week
        return true;
